...
import warnings
import shutil
from cloudshadow_ndvi import cloudshadow
# ignore warnings
...
class change_detection:
    def __init__(self, root, serial,city_name, kmeans_type="TCI_band", n_bands=3, show_plots=False):
        city_params = {
            "tehran"        : {'n_cluster' : 7 , 'ndvi_thre' : 0.3},
            "alborz"        : {'n_cluster' : 6 , 'ndvi_thre' : 0.3},
            'qom'           : {'n_cluster' : 6 , 'ndvi_thre' : 0.2},
            'mazandaran'    : {'n_cluster' : 7 , 'ndvi_thre' : 0.3}
        }
    ...
    def CM(self):  # _____ main function for change detection
        ...
        for i in range(len(image1_paths)):
            ...
            image1t = image1.transpose(1, 2, 0)
            image2t = image2.transpose(1, 2, 0)
            ...
            image1=cloudshadow(image1t)
            image2 =cloudshadow(image2t)

            # _____ for removing nodata from final mask
            
            for row1 in range(image1.shape[0]):
                for col1 in range(image1.shape[1]):
                    if np.any(image1[row1,col1]==0) or np.any(image2[row1,col1]==0):
                        # _____ replacing pixels with pix value of 0 to 1 , these pixels are no data
                        image1[row1,col1]=1
                        image2[row1,col1]=1
            ...
            img2_vis = image1t[:, :, 10:13] .astype(np.uint8)
            ...
            ndvi1_area  = ndvi(image1_new)
            ndvi2_area  = ndvi(image2_new)            
            ...
            ########### post_classification CD ###########

            change_mask_pc = post_CD(self.root, image1_path, image2_path, serial=self.serial)
            # _____________________________________________________________________________
            ...
            final_mask = VOTE(weighted_method, change_mask_kmeans=change_mask_kmeans,
                              change_mask_cva=change_mask_cva, change_mask_pc=change_mask_pc,diff_ndvi=diff_ndvi,  y=y, x=x,
                              ndvi1_area=ndvi1_area,ndvi2_area=ndvi2_area)
            """
            ...
            new_dataset.write(np.array([final_mask]))
            new_dataset.close()
            """
            # _____________________________________________________________________________
            ########### plot ###########
            ...
            if self.show_plots:
                PLOT(weighted_method, img1_vis, img2_vis, change_mask_cva, change_mask_kmeans, change_mask_pc, final_mask,
                     diff_ndvi,diff_ndvig, y, x)
    ...
    def main(self):
        ...
        seg_path = os.path.join(root, 'data/seg')
        chips_result = os.path.join(root, 'data/chips_results')

        result_path = os.path.join(self.root ,'data/results/ai')
        ...
        with rio.open(image_path, 'w', **metadata) as tile_dataset:
            tile_dataset.write(np.array([final_mask]))
            tile_dataset.close()
...
root = os.getcwd()
change_process = change_detection(root, serial='R62C29',city_name='mazandaran',show_plots=True)
change_process.main()
